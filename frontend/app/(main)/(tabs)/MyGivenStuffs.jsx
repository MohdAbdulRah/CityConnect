...
const MyGivenStuffs = () => {
  ...
  const fetchGivenStuffs = async () => {
    try {
      const res = await authApi.get("/api/stuff/my-stuffs");
      if (res.data.success) {
    ...
    return (
      ...
      >
        ...
        <Text className="text-white/80 text-lg mt-2 text-center">
          Start giving items to other users
        </Text>
      </LinearGradient>
    );
  }
  ...
  return (
    <LinearGradient colors={["#0f172a", "#1e293b"]} className="flex-1">
      <SafeAreaView className="flex-1 px-6 pt-6">
        <Text className="text-3xl font-extrabold text-white mb-4">My Given Stuffs</Text>
        <FlatList
          data={stuffs}
