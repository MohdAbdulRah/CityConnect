...
const OtherGivenStuffs = () => {
  ...
  const fetchGivenStuffs = async () => {
    try {
      const res = await authApi.get(`/api/stuff/stuffByUser/${id}`);
      if (res.data.success) {
      ...
      Alert.alert(
        "Error",
        err?.response?.data?.message || "Failed to fetch given stuffs"
      );
    } finally {
      setLoading(false);
    }
    ...
    return (
      ...
      >
        ...
        <Text className="text-white/80 text-lg mt-2 text-center">
          The user has not given any stuff yet
        </Text>
      </LinearGradient>
    );
  }
  ...
  return (
    <LinearGradient colors={["#0f172a", "#1e293b"]} className="flex-1">
      <SafeAreaView className="flex-1 px-6 pt-6">
        <Text className="text-3xl font-extrabold text-white mb-4">Given Stuffs</Text>
        <FlatList
          data={stuffs}
...
export default OtherGivenStuffs;
